...
while ($student = db_fetch($students)) {
	...
	$avg_diff = (array_sum($given_grades_diff)/count($given_grades_diff));
	$coherence = 0;
	for ($i=0; $i<count($given_grades_diff); $i++) {
		$coherence += abs($given_grades_diff[$i]-$avg_diff);
	}
	echo $stud_post['post_id']."\t".$avg_diff."\t".$coherence."\t".$stud_post_grade['g']."\t".$prof_post_grade['g']."\n";
}